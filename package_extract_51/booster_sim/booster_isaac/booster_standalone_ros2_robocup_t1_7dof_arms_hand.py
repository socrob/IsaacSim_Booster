...
sys.path.insert(0, "/home/ltl7/isaacsim/5.1/exts/isaacsim.ros2.bridge/humble/rclpy")
# ── ROS2 Bridge Extension activation ──
...
# ── Stdlib & Third-party  ──
import os, carb, time, math, argparse, threading
import numpy as np
...
class Booster_direct_runner(Node):
    def __init__(self, physics_dt, render_dt, bg_asset_path) -> None:
        ...
        self._ks_robot._pub_joint_state = self.create_publisher(JointState, '/booster/ros2_k2_joint_states', 10)
        self._ks_robot._msg_joint_state = JointState()

        self._ks_robot._pub_real_joint_state = self.create_publisher(JointState, '/joint_states', 10)
        self._ks_robot._msg_real_joint_state = JointState()

        self._ks_robot._pub_imu = self.create_publisher(Imu, '/booster/ros2_k2_imu', 10)
        self._ks_robot._msg_imu = Imu()
        ...
        try:
            ...
            (self._ros_camera_graph, _, _, _) = og.Controller.edit(
                ...
                },
                {
                    keys.CREATE_NODES: [
                        ...
                        ("cameraLeftHelperRgb", "isaacsim.ros2.bridge.ROS2CameraHelper"),
                        ("createRightRenderProduct", "isaacsim.core.nodes.IsaacCreateRenderProduct"),
                        ("cameraRightHelperRgb", "isaacsim.ros2.bridge.ROS2CameraHelper"),
                        ("createLeftRenderProductInfo", "isaacsim.core.nodes.IsaacCreateRenderProduct"),
                        ("cameraLeftHelperInfo", "isaacsim.ros2.bridge.ROS2CameraHelper"),
                        ("createRightRenderProductInfo", "isaacsim.core.nodes.IsaacCreateRenderProduct"),
                        ("cameraRightHelperInfo", "isaacsim.ros2.bridge.ROS2CameraHelper"),
                        ("createRenderProductDepthCloud", "isaacsim.core.nodes.IsaacCreateRenderProduct"),
                        ("cameraRenderHelperDepthCloud", "isaacsim.ros2.bridge.ROS2CameraHelper"),
                    ],
                    keys.CONNECT: [
            ...
            # ── NEU: Zweites Viewport-Fenster für die Roboter-Kamera ──────────────────
            try:
                import omni.kit.viewport.utility as vp_utils

                _ROBOT_CAM_PRIM = (
                    robot_prim + "/H2/Realsense/RSD455/Camera_OmniVision_OV9782_Right"
                )

                # Viewport 0 (Standard) bleibt die freie Perspektivkamera.
                # Viewport 1 zeigt die Realsense Color-Kamera des Booster T1.
                robot_cam_vp = vp_utils.create_viewport_window(
                    "Booster T1 – Robot Camera",
                    width=640,
                    height=360,
                )
                robot_cam_vp.viewport_api.set_active_camera(_ROBOT_CAM_PRIM)

                print(f"[ROS Camera Graph] Robot camera viewport created → {_ROBOT_CAM_PRIM}")

            except Exception as vp_err:
                # Nicht-kritisch: ROS2-Publishing läuft weiter, nur die GUI-Ansicht fehlt
                print(f"[ROS Camera Graph] Warning: Could not create robot camera viewport: {vp_err}")
            # ─────────────────────────────────────────────────────────────────────────
    ...
    def hand_control_callback(self, data: Float32MultiArray):
        """
